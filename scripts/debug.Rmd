...
# Set debug folder 
debug_folder <- "D:/Jakob/dk_nationwide_lidar/scratch/debug"
...
analyse_raster <- function(index){
  ...
  raster_plot <- levelplot(target_raster,
             margin = F)
  # Histogram Plot
  hist_plot <- ggplot(as.data.frame(target_raster), 
                      aes(x = get(paste0(variable_name, "_", tile_id)))) +
...
if("aspect" %in% variables & "slope" %in% variables & length(tile_ids) == 1){
  ...
  hill_shade <- hillShade(raster_list[[which(variables == "aspect")]] * pi / 180, 
                          raster_list[[which(variables == "slope")]] * pi / 180)
  cat(paste0("\n## Hillshade ", tile_ids, "\n"))
